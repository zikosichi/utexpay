...
import { PANELS } from '#/components/accountshero/panels'
import { BLEED, PANEL_SURFACES } from '#/components/accountshero/surfaces'
const PANEL_NAMES = ['personal', 'business', 'payments'] as const
 ...
 * transparent page — `scripts/bake-accounts-panels.mjs` screenshots exactly that.
...
const FACES = ['400 16px "DM Sans"', '500 16px "DM Sans"', '400 16px "IBM Plex Mono"']

export const Route = createFileRoute('/accounts-panel-bake')({
  validateSearch: (search: Record<string, unknown>): { surface?: PanelName } => {
    const surface = search.surface
    return typeof surface === 'string' && PANEL_NAMES.includes(surface as PanelName) ? { surface: surface as PanelName } : {}
  },
  component: PanelBake,
  head: () => ({
...
function PanelBake() {
  ...
  if (surface) {
    const spec = PANEL_SURFACES[surface], Panel = PANELS[surface]
    return <>
      ...
      <div data-bake={surface} style={{ position: 'relative', boxSizing: 'content-box', width: spec.width, height: spec.height, padding: BLEED }}>
        <div style={{ position: 'relative', width: spec.width, height: spec.height }}><Panel /></div>
      </div>
    </>
  }
  return <main style={{ padding: 40, background: '#1f1b16', color: '#a89a80', font: '12px/1.5 "IBM Plex Mono", monospace' }}>
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 40, alignItems: 'flex-start' }}>
      {PANEL_NAMES.map((name) => {
        const spec = PANEL_SURFACES[name], Panel = PANELS[name]
        return <figure key={name} style={{ margin: 0 }}>
          <div style={{ position: 'relative', width: spec.width, height: spec.height, outline: '1px dashed #4a4032' }}><Panel /></div>
