...
const PHONE_FRAMINGS: [PhotoFocus, string][] = [['full', 'All'], ['personal', 'Personal'], ['business', 'Business'], ['payments', 'Payments']]

export function LiveAccountsHero() {
  ...
  // Saved picks (fonts, headline, card tuning) are review-mode state: `/` always shows the
  // approved defaults and never writes over what a reviewer tuned under `?studio`.
  useEffect(() => {
    if (!isStudio()) return
    try {
  ...
  useEffect(() => {
    if (!settingsLoaded || !isStudio()) return
    try {
  ...
  // A framing change swaps the stage's aspect ratio (phones: 2.15 wide strip -> 1.1 near-square),
  // which would snap its height — and everything under it — in one frame while the camera eases.
  // Read the height the stage has now, before React commits the new class (mid-tween if the
  // visitor taps again), then tween to the new height. The scene's ResizeObserver follows along.
  const framedFocus = useRef(options.focus), framingFrom = useRef<number | null>(null)
  if (framedFocus.current !== options.focus) {
    framedFocus.current = options.focus
    if (stage.current) framingFrom.current = stage.current.getBoundingClientRect().height
  }
  useLayoutEffect(() => {
    const element = stage.current, from = framingFrom.current
    framingFrom.current = null
    if (!element || from === null) return
    const surface = canvas.current!
    element.getAnimations().forEach((animation) => { if (animation.id === 'framing') animation.cancel() })
    ...
    // Hold the canvas at the taller of the two hero heights (the hero clips it) so the WebGL
    // buffers are reallocated once, not on every frame of the tween.
    const heroHeight = element.parentElement!.getBoundingClientRect().height
    surface.style.height = `${Math.max(heroHeight, heroHeight + from - to)}px`
    const tween = element.animate([{ height: `${from}px` }, { height: `${to}px` }], { id: 'framing', duration: 560, easing: 'cubic-bezier(.22, 1, .36, 1)' })
    tween.onfinish = () => { surface.style.height = '' }
  }, [options.focus])
  const hints = useRef<{ update(settings: HintSettings): void; stop(): void } | null>(null)
  useEffect(() => {
    if (status === 'loading') return
    // The cue waits for the scene to hand the panels over (they are inert during the intro), and
    // schedules nothing while the hero is scrolled away.
    const panels = htmlPanels.current!
    const started = startIdleNudge(panels, canvas.current!, () => !panels.inert && !!panels.closest('.accounts-hero.is-onscreen'), optionsRef.current)
    hints.current = started
    return () => { started.stop(); hints.current = null }
  }, [status])
  useEffect(() => { hints.current?.update(options) }, [options])
  // `is-onscreen` lets the idle nudge rest while the hero is scrolled away.
  useEffect(() => {
    const hero = htmlPanels.current?.closest('.accounts-hero')
    if (!hero || typeof IntersectionObserver === 'undefined') return
    const observer = new IntersectionObserver(([entry]) => hero.classList.toggle('is-onscreen', entry.isIntersecting))
  ...
  function demo(fromAway = false) {
    reset()
    setOptions((previous) => ({ ...previous, mode: 'photo', focus: 'full' }))
    stage.current?.focus({ preventScroll: true })
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    if (fromAway) window.scrollTo({ top: 0, behavior: reduced ? 'instant' : 'smooth' })
    else stage.current?.scrollIntoView({ behavior: reduced ? 'instant' : 'smooth', block: 'nearest' })
  ...
  // "Try live demo" further down the page links to /#demo: come back up and play the same demo.
  const demoFromAway = useRef(demo)
  demoFromAway.current = demo
  useEffect(() => {
    function play() {
      if (window.location.hash !== '#demo') return
      history.replaceState(null, '', window.location.pathname + window.location.search)
      demoFromAway.current(true)
    }
    play()
    window.addEventListener('hashchange', play)
    return () => window.removeEventListener('hashchange', play)
  }, [])
  ...
  return (
    <section className="studio photo-hero accounts-hero" aria-label="Start with an account" style={heroFontStyle(font, bodyFont)} onPointerMove={move} onPointerLeave={reset}>
      ...
      {studio && <HeadlineSwitcher value={headline} onChange={setHeadline} />}
      ...
      {/* Phones: tapping a box zooms into it, so the framing needs a visible way back and across. */}
      <div className="ah-phone-framing" role="group" aria-label="Show account">
        {PHONE_FRAMINGS.map(([value, label]) => <button key={value} type="button" aria-pressed={options.focus === value}
          onClick={() => { reset(); setOptions((previous) => ({ ...previous, focus: value })) }}>{label}</button>)}
      </div>
      {studio && <div className="studio-tools photo-tools" onPointerEnter={reset} onKeyDown={(event) => {
