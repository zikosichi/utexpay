...
import type { PhotoFocus, PhotoOptions, PhotoScene, ViewMode } from './config'
...
export function PhotoStructure() {
  ...
  const [backgroundLight, setBackgroundLight] = useState(1)
  const [backgroundLightSpread, setBackgroundLightSpread] = useState(1)
  ...
  useEffect(() => {
    ...
    createPhotoScene(element, fallback.current!, controller.signal).then((created) => {
  ...
  return (
    <main className="studio photo-hero" style={{
      '--photo-light-opacity': Math.min(backgroundLight, 1),
      '--photo-light-brightness': Math.max(backgroundLight, 1),
      '--photo-light-spread': backgroundLightSpread,
    } as CSSProperties} onPointerMove={move} onPointerLeave={reset}>
      <HeroNavigation />
      ...
      <div className="studio-tools photo-tools" onPointerEnter={reset} onKeyDown={(event) => {
        ...
        {controls && <aside id="photo-controls" className="studio-controls photo-controls" aria-label="Hero configuration">
          ...
          <label className="studio-range">Background light <output>{Math.round(backgroundLight * 100)}%</output><input aria-label="Background light" type="range" min="0" max="1.6" step="0.05" value={backgroundLight} onChange={(event) => setBackgroundLight(Number(event.target.value))} /></label>
          <label className="studio-range">Light spread <output>{Math.round(backgroundLightSpread * 100)}%</output><input aria-label="Light spread" type="range" min="0.7" max="1.5" step="0.05" value={backgroundLightSpread} onChange={(event) => setBackgroundLightSpread(Number(event.target.value))} /></label>
          ...
          <button className="studio-restore" onClick={() => { reset(); setOptions(DEFAULT_OPTIONS); setBackgroundLight(1); setBackgroundLightSpread(1) }}>Restore defaults</button>
