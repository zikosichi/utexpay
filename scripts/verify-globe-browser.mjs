...
try {
  ...
  const migration = await send('Page.addScriptToEvaluateOnNewDocument', { source: `localStorage.removeItem('utexpay.globe.appearance.v5');localStorage.setItem('utexpay.globe.appearance.v4', JSON.stringify({size:55,density:100,radius:130}));` })
  ...
  assert.equal(await evaluate(`${canvas}.dataset.dotSize`), '50', '50% base replaces the old saved size')
  assert.equal(await evaluate(`${canvas}.dataset.hoverRadius`), '25', '25% radius replaces the old saved radius')
  ...
  assert.ok(Math.abs(await evaluate(`Number(${canvas}.dataset.hoverPixels)`) - originalRadius * 150 / 25) < .1, 'radius slider updates the live hover field')
  await setRange('size', 150)
  assert.ok(Math.abs(await evaluate(`Number(${canvas}.dataset.dotPixels)`) / originalSize - 150 / 50) < .002, 'dot size updates at the current viewport')
  ...
  // The second distribution is fetched on first use, so the count arrives asynchronously.
  await until(`Number(${canvas}.dataset.points) > ${pointCount * 1.98}`, 'density adds a second geographic distribution')
  ...
  await setRange('size', 50); await setRange('density', 100); await setRange('radius', 25)
  ...
  for (const [width, height] of [[1920, 1040], [1440, 806], [768, 760], [390, 760], [320, 760]]) {
    ...
    layouts.push(layout)
    // The numbers row makes the section taller than these viewports; bring the controls on screen first.
    await evaluate(`document.querySelector('.gh-settings').scrollIntoView({ block: 'end', behavior: 'instant' })`)
