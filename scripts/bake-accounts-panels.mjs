...
//
//   node --experimental-strip-types scripts/bake-accounts-panels.mjs [--scale 3] [--quality 92] [--only a,b] [--base http://127.0.0.1:3000] [--png]
//
// Needs the dev server running. Each panel is rendered flat by
// `/accounts-panel-bake?surface=<name>` over a transparent page, screenshotted by
// headless Chrome at `scale` device pixels per CSS pixel, and encoded to
// `public/accountshero/panels/<name>.webp` (lossy RGB, lossless alpha).
// `--png` keeps the PNG next to it for inspection.
import { createHash } from 'node:crypto'
import { spawn, spawnSync } from 'node:child_process'
import { mkdirSync, readFileSync, statSync, unlinkSync, writeFileSync } from 'node:fs'
import { join } from 'node:path'
import { BLEED, PANEL_SURFACES } from '../src/components/accountshero/surfaces.ts'

import { PANEL_VERSIONS } from '../src/components/accountshero/panelVersions.ts'

const versions = { ...PANEL_VERSIONS }
const PANEL_NAMES = ['personal', 'business', 'payments']
...
const outDir = join(import.meta.dirname, '..', 'public', 'accountshero', 'panels')
...
for (const name of names) {
  ...
  await send('Page.navigate', { url: `${base}/accounts-panel-bake?surface=${name}` })
  let ready = ''
  ...
  versions[name] = createHash('sha256').update(readFileSync(webp)).digest('hex').slice(0, 12)
...
if (!failed) writeFileSync(join(import.meta.dirname, '..', 'src', 'components', 'accountshero', 'panelVersions.ts'), '// Generated by scripts/bake-accounts-panels.mjs.\nexport const PANEL_VERSIONS = ' + JSON.stringify(versions, null, 2) + ' as const\n')
